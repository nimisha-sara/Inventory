...
def sales():
    if (request.method == 'POST') and 'delete' in request.form:
    ...
    elif (request.method == 'POST') and 'print' in request.form:
        ...
        products = [request.form[f"prod-{i}"] for i in range(0, 11)
                    if request.form[f"prod-{i}"] != '']
        product_count = len(products)

        quantity = [int(request.form[f"quant-{i}"]) for i in range(0, 11)
                    if request.form[f"prod-{i}"] != 0]
        rates = [int(request.form[f"rate-{i}"]) for i in range(0, 11) if
                 request.form[f"prod-{i}"] != 0]
        total = sum([(quantity[i] * rates[i]) for i in range(0, 10)])

        products, quantity = "$$".join(products), ','.join(map(str, quantity))
        ...
        if 'edit-sale' in request.form:
            ...
            database.update(_id, customer=customer, address=address,
                            contact=contact, products=products,
                            quantity=quantity, rates=rates, total=total,
                            product_count=product_count, city_state=city_state)
